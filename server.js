...
async function loadKnowledgeBase() {
  ...
  if (fs.existsSync(textKnowledgePath)) {
    try {
      ...
      console.error("knowledge.txt error:", e.message);
    }
  }
  ...
  for (const url of PDF_URLS) {
    ...
    const safeName = `${Date.now()}-${Math.random().toString(36).slice(2)}-${name.replace(/[^a-zA-Z0-9._-]/g, "_")}`;
    const localPath = path.join(os.tmpdir(), safeName);

    try {
      const buffer = await downloadPdf(url, localPath);
      let extractedText = "";
      let pages = 0;

      try {
        const pdf = await pdfParse(buffer);
        extractedText = normalizeText(pdf.text);
        pages = pdf.numpages || 0;
      } catch (e) {
        console.warn(`Text parser failed: ${name}: ${e.message}`);
      }

      // IMPORTANT: Even if pdf-parse cannot read the PDF, upload the original PDF
      // to Gemini. Gemini can read scanned/image-based PDFs.
      ...
      if (process.env.GEMINI_API_KEY) {
        try {
          ...
          console.log(`Gemini PDF uploaded: ${name}`);
        } catch (e) {
          console.error(`Gemini PDF upload failed: ${name}: ${e.message}`);
        }
      }

      documents.push({ name, text: extractedText, url, pages, uploadedFile });
      pdfFiles.push({ name, url, uploadedFile });
      pdfCount++;

      if (extractedText) {
        console.log(`PDF loaded: ${name} | pages=${pages} | chars=${extractedText.length}`);
      } else {
        console.log(`PDF loaded for Gemini vision/OCR: ${name} | text parser found no text`);
      }
  ...
  for (const doc of documents) {
    ...
    for (const text of makeChunks(doc.text)) {
      chunks.push({ source: doc.name, url: doc.url || null, text });
    }
  }
...
function tokenize(text) {
  return normalizeText(text).toLowerCase().replace(/[^a-z0-9\u0900-\u097f\s]/gi, " ").split(/\s+/).filter(w => w.length >= 2 && !STOP_WORDS.has(w));
}

function sourceMatches(question, name) {
  const q = normalizeText(question).toLowerCase();
  const n = name.toLowerCase();
  if (/meena\s*(manch|munch)|मीना\s*मंच/i.test(q)) return /meena\s*munch|meena\s*manch/i.test(n);
  if (/cwpc/i.test(q)) return /cwpc/i.test(n);
  if (/child\s*trafficking|trafficking|बाल\s*तस्करी|तस्करी/i.test(q)) return /trafficking/i.test(n);
  if (/kawach|कवच/i.test(q)) return /kawach/i.test(n);
  if (/shg|self.?help\s*group/i.test(q)) return /shg/i.test(n);
  if (/yojana|योजना|scheme/i.test(q)) return /yojana/i.test(n);
  if (/bal\s*sanrakshan|बाल\s*संरक्षण/i.test(q)) return /bal\s*sanrakshan/i.test(n);
  return false;
}

function searchTextChunks(question) {
  const queryTokens = [...new Set(tokenize(question))];
  if (!queryTokens.length) return chunks.slice(0, 5);
  return chunks.map(chunk => {
    ...
    for (const token of queryTokens) {
      ...
      let count = 0, pos = 0;
      while ((pos = text.indexOf(token, pos)) !== -1) { count++; pos += token.length; if (count >= 4) break; }
...
function selectGeminiPdfs(question) {
  const exact = pdfFiles.filter(file => sourceMatches(question, file.name));
  if (exact.length) return exact.slice(0, 2).filter(x => x.uploadedFile);
  return [];
}
...
async function generateGeminiResponse(message) {
  const selectedPdfs = selectGeminiPdfs(message);
  const textMatches = searchTextChunks(message);
  const textContext = textMatches.length ? textMatches.map((x,i)=>`TEXT SOURCE ${i+1}: ${x.source}\n${x.text}`).join("\n\n---\n\n") : "No searchable text excerpt found.";

  const sourceHint = selectedPdfs.length
    ? `The user is asking about: ${selectedPdfs.map(x=>x.name).join(", ")}. Use these PDF files as the primary source.`
    : "Use the relevant supplied text excerpts and any supplied PDF file when applicable.";

  const contents = [];
  for (const file of selectedPdfs) {
    contents.push(createPartFromUri(file.uploadedFile.uri, file.uploadedFile.mimeType));
  }
  contents.push(`${sourceHint}\n\nQuestion: ${message}\n\nRelevant extracted text (may be incomplete for scanned PDFs):\n${textContext}`);
...
app.post("/api/chat", async (req,res) => {
  try {
    ...
    if (message.length > 4000) return res.status(400).json({error:"Question is too long. Please keep it under 4000 characters."});
    ...
    return res.status(503).json({error:"KAWACH is temporarily busy. Please try again in a few seconds. If you need immediate help, contact 1098 or 112."});
  }
});
...
async function startServer(){
  ...
  app.listen(port,"0.0.0.0",()=>{
    ...
    console.log(`Gemini PDF files: ${pdfFiles.filter(x=>x.uploadedFile).length}`);
  });
}
